  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resp?.status == 200) {
        ...
        setTitle("");
        setDescription("");
        toast.success("Se cargo tu imagen!!!!")
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8 shadow-2xl">
        ...
        <form onSubmit={submitForm} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              {preview ? (
                ...
                <>
                  <svg
                    className="w-10 h-10 text-slate-500 group-hover:text-indigo-400 mb-3 transition"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
          ...
          <button
            type="submit"
            className="mt-2 bg-indigo-600 hover:bg-indigo-500  text-white font-semibold py-2.5 rounded-lg transition cursor-pointer text-center"
            disabled={loading}
          >
            {loading ? (
